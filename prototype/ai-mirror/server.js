// AI Mirror 프로토타입 서버 (v0.2)
// 역할: ① public/ 폴더의 화면 파일 제공  ② 브라우저가 보낸 사진을 Claude에 전달해 해석+추천 받기
...
// 제품 카탈로그 (public/catalog.json — 화면과 서버가 같은 파일을 씁니다)
const catalog = JSON.parse(
  readFileSync(path.join(__dirname, "public", "catalog.json"), "utf8"),
);
const PRODUCT_IDS = catalog.products.map((p) => p.id);
const CATALOG_TEXT = catalog.products
  .map((p) => `- id: ${p.id} / ${p.name} / ${p.frame} / 무드: ${p.mood}`)
  .join("\n");

const SYSTEM_PROMPT = `당신은 젠틀몬스터 매장에 설치된 'AI Mirror'입니다.
거울 앞에 선 사람의 사진을 보고, 그 사람을 시적으로 해석하고 어울리는 제품을 고릅니다.

해석 원칙:
1. '근거 있는 시' 방식을 씁니다: 사진에서 실제로 관찰 가능한 사실 하나(앵커)를 찾고,
   그 사실이 자연스럽게 스며든 시적인 한 문장을 만듭니다.
   예: "오늘의 당신은 차분한 직선 — 어깨의 각이 그렇게 말합니다."
2. 외모를 평가하지 않습니다. (예쁘다, 잘생겼다, 피부, 체형 등 금지)
   대신 인상, 분위기, 선, 색, 빛, 자세 같은 것을 읽습니다.
3. 사람의 신원을 추측하거나 특정하지 않습니다.
4. sentence는 한국어 한 문장, 15~45자. 과장 없이, 낯설지만 다정하게.
5. anchor는 사진에서 관찰한 사실을 아주 짧게. (예: "왼쪽으로 살짝 기운 시선")
6. mood_word는 무드를 담은 한 단어. (예: "고요", "직선", "새벽")
7. mood_tone은 화면 연출용 톤 분류입니다. 사진의 분위기에 가장 가까운 것을 고르세요.

추천 원칙:
- 아래 카탈로그 안에서만 고릅니다. 카탈로그에 없는 제품을 만들어내지 않습니다.
- 읽어낸 무드·얼굴의 선과 어울리는 제품 id를 정확히 3개, 어울리는 순서대로 고릅니다.
- 추천 이유는 쓰지 않습니다. (화면에는 목록만 표시됩니다)
...
const RESULT_SCHEMA = {
  ...
  properties: {
    ...
    recommendations: {
      ...
      description: "무드와 어울리는 제품 id 정확히 3개, 어울리는 순서대로",
    },
  },
  required: ["anchor", "mood_word", "mood_tone", "sentence", "recommendations"],
  additionalProperties: false,
};

// 데모 모드에서 순환하며 보여줄 결과들
const DEMO_RESULTS = [
  {
    ...
    recommendations: ["her-01", "pino-01", "liliit-01"],
  },
  {
    anchor: "부드럽게 내려온 어깨",
    mood_word: "고요",
    ...
    recommendations: ["jade-01", "lang-01", "malta-01"],
  },
  {
    ...
    recommendations: ["lang-01", "papas-01", "jade-01"],
  },
];
let demoIndex = 0;
    ...
    messages: [
      {
        ...
        content: [
          ...
          { type: "text", text: "지금 거울 앞에 선 사람을 해석하고, 어울리는 제품을 골라 주세요." },
        ],
      },
    ],
  });
...
async function handleInterpret(req, res) {
  ...
  try {
    ...
    console.log(`[live] 해석 완료 ${Date.now() - started}ms — "${result.sentence}" / 추천: ${result.recommendations.join(", ")}`);
  } catch (error) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && req.url === "/api/health") {
    sendJson(res, 200, { ok: true, forced_demo: FORCE_DEMO, model: MODEL, effort: EFFORT, products: PRODUCT_IDS.length });
    return;
  }
...
server.listen(PORT, "127.0.0.1", () => {
  console.log("");
  console.log("  AI MIRROR 프로토타입 v0.2");
  console.log(`  → 브라우저에서 열기: http://localhost:${PORT}`);
  console.log(`  → 모드: ${FORCE_DEMO ? "데모 (API 호출 없음)" : "라이브 시도 (인증 실패 시 자동으로 데모 전환)"}`);
  console.log(`  → 카탈로그: ${PRODUCT_IDS.length}개 제품`);
  console.log("");
});
